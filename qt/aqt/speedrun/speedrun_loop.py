...
POINTS_WEIGHTS: dict[str, float] = {
    ...
    # CARS is ~53/59 of a section relative to P/S; treated as equal weight.
    "CARS": 1.0,
}

# Tunable constants for the performance-based question weight formula.
# weight = section_weight * max(1 - performance, QUESTION_WEIGHT_FLOOR)
#          * (1 + QUESTION_DECAY_RATE * days_since_last_practiced)
...
class TopicStats:
    ...
    answered: int
    # Days since the most recent answered question; None if never practiced.
    last_practiced_days_ago: float | None = None
    # False for topics with no flashcard deck (e.g. CARS); they skip the
    # memory eligibility gate and are always question-eligible.
    has_flashcards: bool = True
    ...
    def points_at_stake(self) -> float:
        """Memory-based urgency; used for flashcard block selection."""
        return self.weight * self.weakness

    @property
    def question_weight(self) -> float:
        """Performance + recency urgency; used for question block selection.
        ...
        * recency_factor = 1 + DECAY_RATE × days_since_last_practiced
          ...
          retrievability decay for question practice.
        """
        deficit = (
        ...
        days = (
            ...
            if self.last_practiced_days_ago is not None
            else QUESTION_DECAY_DEFAULT_DAYS
        )
        return self.weight * deficit * (1.0 + QUESTION_DECAY_RATE * days)
    ...
    def is_question_eligible(self) -> bool:
        ...
        Topics without a flashcard deck (e.g. CARS) are always eligible.
        For flashcard topics, application practice only makes sense once memory
        is established: the student must have reviewed at least
        ``QUESTION_REVIEWED_THRESHOLD`` cards AND have memory above
        ``QUESTION_MEMORY_THRESHOLD``. Below either bar the topic gets
        flashcards first.
        """
    ...
    def has_knowledge_gap(self) -> bool:
        # Topics without flashcards never have a knowledge gap (no cards to
        # review). For flashcard topics, anything not yet question-eligible is
        # a knowledge gap: the student must build memory first.
        if not self.has_flashcards:
            return False
...
def build_topic_stats(col: Collection, topics: list[str]) -> dict[str, TopicStats]:
    ...
    last_practiced = performance_score.fetch_topic_last_practiced(col)

    stats: dict[str, TopicStats] = {}
    for name in topics:
        section = memory_score.SUBDECK_TO_SECTION.get(name, "")
        has_flashcards = name not in memory_score.QUESTIONS_ONLY_TOPICS
        ...
        stats[name] = TopicStats(
            ...
            answered=answered,
            last_practiced_days_ago=last_practiced.get(name),
...
def highest_points_at_stake(
    ...
    *,
    ...
    When ``flashcards_only`` is True, topics without a flashcard deck (e.g.
    CARS) are excluded so they are never picked for flashcard block planning.
    """
    exclude = exclude or set()
    candidates = [
        t
        for t in stats.values()
        if t.name not in exclude and (not flashcards_only or t.has_flashcards)
    ]
    if not candidates:
        return None
...
def weighted_interleave(
    ...
    *,
    ...
    """Pick a mixed block of ids, weighting topics by a caller-supplied weight.

    ``pools`` maps a topic to the ids available for it (question ids or card
    ids). Topics with higher weight are drawn more often within the block.
    Topics with no remaining ids are skipped. If ``include_topic`` has ids,
    at least one of them is guaranteed in the block (used to re-surface a
    just-remediated topic).

    ``weight_fn`` is an optional callable that returns the weight for a topic
    name. Defaults to ``points_at_stake`` (memory-based) so flashcard block
    selection is unchanged; question blocks pass ``question_weight`` instead.
    """
    ...
    def default_stake(topic: str) -> float:
        ts = stats.get(topic)
        return max(ts.points_at_stake, 1e-6) if ts is not None else 1e-6

    get_weight = weight_fn if weight_fn is not None else default_stake
    ...
    while len(chosen) < size:
        ...
        if not available:
            break
        weights = [max(get_weight(t), 1e-6) for t in available]
...
def select_interleaved_questions(
    ...
    *,
    ...
    """Pick a mixed block of question ids, weighted by performance + recency.

    Only question-eligible topics are included. For flashcard topics this means
    memory must be established (>= QUESTION_REVIEWED_THRESHOLD reviewed cards
    AND memory > QUESTION_MEMORY_THRESHOLD). Topics without a flashcard deck
    (e.g. CARS) are always eligible.

    Weighting uses ``question_weight`` (performance-based + time-decay) rather
    than ``points_at_stake`` (memory-based), so topics where the student
    answers questions poorly AND hasn't practiced recently get drawn more often.
    """
    ...
    return weighted_interleave(
        ...
        rng=rng,
        weight_fn=lambda t: stats[t].question_weight if t in stats else 1e-6,
    )
...
class SpeedrunSession:
    ...
    def _plan_opening_flashcards(
        ...
        topic = highest_points_at_stake(
            stats, exclude=self.exhausted_topics, flashcards_only=True
        )
        if topic is None:
            return None
        return BlockPlan(
    ...
    def _fallback_to_flashcards(
        ...
        (flashcard topics only) so the student builds recall before practice.
        """
